...
export class CampaignsService {
  ...
  async findAll(
    pageOptionsDto: CampaignsPageOptionsDto,
  ): Promise<PageDto<Campaign>> {
    const queryBuilder =
    ...
    if (pageOptionsDto.status) {
      if (Array.isArray(pageOptionsDto.status)) {
        queryBuilder.andWhere('campaign.status IN (:...statuses)', {
          statuses: pageOptionsDto.status,
        });
      } else {
    ...
    if (pageOptionsDto.minGoal) {
      queryBuilder.andWhere('campaign.goal_amount >= :minGoal', {
        minGoal: pageOptionsDto.minGoal,
      });
    }

    if (pageOptionsDto.maxGoal) {
      queryBuilder.andWhere('campaign.goal_amount <= :maxGoal', {
        maxGoal: pageOptionsDto.maxGoal,
      });
    }

    queryBuilder
      .orderBy(
        `campaign.${pageOptionsDto.sortBy || 'created_at'}`,
