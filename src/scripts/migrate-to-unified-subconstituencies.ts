...
async function migrateSubConstituencies() {
  const dataSource = new DataSource({
    ...
    entities: [Donor, Constituency, SubConstituency, Class, Teacher],
  ...
  try {
    ...
    const studentsConst = await dataSource
      .getRepository(Constituency)
      .findOneBy({ name: 'Students' });
    const teachersConst = await dataSource
      .getRepository(Constituency)
      .findOneBy({ name: 'Teaching Staff' });

    if (!studentsConst || !teachersConst) {
      console.error(
        'Critical constituencies not found! Ensure "Students" and "Teaching Staff" exist.',
      );
      return;
    }
    ...
    const allConstituencies = await dataSource
      .getRepository(Constituency)
      .find();
    ...
    allSubs.forEach((s) =>
      subConstituencyCache.set(`${s.constituency_id}-${s.name}`, s),
    );
    ...
    for (const donor of donors) {
      ...
      if (
        ...
        !donor.sub_constituency_entity &&
        donor.constituency_entity
      ) {
        ...
        if (subEntity) {
          ...
          console.log(
            `Creating missing sub-constituency: ${donor.sub_constituency} for ${donor.constituency}`,
          );
