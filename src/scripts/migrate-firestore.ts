...
const EXPORT_DIR =
  '/Users/Apple/Desktop/development-work/firestore-migration/exported_data';
...
async function migrate() {
  const dataSource = new DataSource({
    ...
    entities: [User, Campaign, Donor, Donation, Constituency, Class, Teacher],
  ...
  try {
    ...
    if (fs.existsSync(constituenciesDir)) {
      const files = fs
        .readdirSync(constituenciesDir)
        .filter((f) => f.endsWith('.json'));
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(constituenciesDir, file), 'utf-8'),
        );
        if (!data.name) continue;

        let constituency = await dataSource
    ...
    if (fs.existsSync(classesDir)) {
      ...
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(classesDir, file), 'utf-8'),
        );
        if (!data.name) continue;

        let cls = await dataSource
          .getRepository(Class)
          .findOneBy({ name: data.name });
        if (!cls) {
    ...
    if (fs.existsSync(teachersDir)) {
      ...
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(teachersDir, file), 'utf-8'),
        );
        if (!data.name) continue;

        // Check by name as we don't strictly use ID here yet
        let teacher = await dataSource
    ...
    if (fs.existsSync(usersDir)) {
      ...
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(usersDir, file), 'utf-8'),
        );
    ...
    if (fs.existsSync(campaignsDir)) {
      ...
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(campaignsDir, file), 'utf-8'),
        );
        ...
        if (!campaign) {
          campaign = dataSource.getRepository(Campaign).create({
            ...
            start_date:
              convertFirestoreDate(data.startDate)
                ?.toISOString()
                .split('T')[0] || undefined,
            end_date:
              convertFirestoreDate(data.endDate)?.toISOString().split('T')[0] ||
              undefined,
            status: data.status,
    ...
    if (fs.existsSync(donorsDir)) {
      ...
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(donorsDir, file), 'utf-8'),
        );
        if (!data.email) continue;

        let donor = await dataSource
          .getRepository(Donor)
          .findOneBy({ email: data.email });
        if (!donor) {
    ...
    if (fs.existsSync(donationsDir)) {
      const files = fs
        .readdirSync(donationsDir)
        .filter((f) => f.endsWith('.json'));
      for (const file of files) {
        const data = JSON.parse(
          fs.readFileSync(path.join(donationsDir, file), 'utf-8'),
        );
