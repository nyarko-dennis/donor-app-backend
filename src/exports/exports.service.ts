...
export class ExportsService {
  ...
  private async getDonors(filters?: ExportFiltersDto): Promise<any[]> {
    ...
    return donors.map((d) => {
      ...
      return {
        ...
        'Sub-Constituency':
          d.sub_constituency || d.sub_constituency_entity?.name,
  ...
  private async getCampaigns(filters?: ExportFiltersDto): Promise<any[]> {
    const queryBuilder =
      this.campaignsRepository.createQueryBuilder('campaign');

    // Apply simple filters if any applicable (Campaigns might have fewer filters)
    if (filters?.search) {
      queryBuilder.where('campaign.name ILIKE :search', {
    ...
    return campaigns.map((c) => {
      ...
      return {
        ...
        'Created Time': createdAt
          ? createdAt.toISOString().split('T')[1].split('.')[0]
          : '',
      };
    });
  }

  private applyDonationFilters(
    qb: SelectQueryBuilder<Donation>,
    filters: ExportFiltersDto,
  ) {
    if (filters.search) {
      qb.andWhere(
        '(donation.payment_method ILIKE :search OR donor.first_name ILIKE :search OR donor.last_name ILIKE :search OR donor.email ILIKE :search)',
        { search: `%${filters.search}%` },
      );
    }
    if (filters.startDate) {
      qb.andWhere('donation.donation_date >= :startDate', {
        startDate: filters.startDate,
      });
    }
    if (filters.endDate) {
      qb.andWhere('donation.donation_date <= :endDate', {
        endDate: filters.endDate,
      });
    }
    if (filters.minAmount) {
      qb.andWhere('donation.amount >= :minAmount', {
        minAmount: filters.minAmount,
      });
    }
    if (filters.maxAmount) {
      qb.andWhere('donation.amount <= :maxAmount', {
        maxAmount: filters.maxAmount,
      });
    }
    if (filters.campaignId) {
      if (Array.isArray(filters.campaignId)) {
        qb.andWhere('donation.campaign.id IN (:...campaignIds)', {
          campaignIds: filters.campaignId,
        });
      } else {
        qb.andWhere('donation.campaign.id = :campaignId', {
    ...
    if (filters.causeId) {
      if (Array.isArray(filters.causeId)) {
        qb.andWhere('donation.cause.id IN (:...causeIds)', {
          causeIds: filters.causeId,
        });
      } else {
        qb.andWhere('donation.cause.id = :causeId', {
    ...
    if (filters.donorId) {
      if (Array.isArray(filters.donorId)) {
        qb.andWhere('donation.donor.id IN (:...donorIds)', {
          donorIds: filters.donorId,
        });
      } else {
        qb.andWhere('donation.donor.id = :donorId', {
    ...
    if (filters.constituencyId) {
      if (Array.isArray(filters.constituencyId)) {
        qb.andWhere('donation.constituency.id IN (:...constituencyIds)', {
          constituencyIds: filters.constituencyId,
        });
      } else {
        qb.andWhere('donation.constituency.id = :constituencyId', {
    ...
    if (filters.subConstituencyId) {
      if (Array.isArray(filters.subConstituencyId)) {
        qb.andWhere(
          'donation.sub_constituency.id IN (:...subConstituencyIds)',
          { subConstituencyIds: filters.subConstituencyId },
        );
      } else {
        qb.andWhere('donation.sub_constituency.id = :subConstituencyId', {
          subConstituencyId: filters.subConstituencyId,
        });
      }
    }
  }

  private applyDonorFilters(
    qb: SelectQueryBuilder<Donor>,
    filters: ExportFiltersDto,
  ) {
    if (filters.search) {
      qb.andWhere(
    ...
    if (filters.constituencyId) {
      if (Array.isArray(filters.constituencyId)) {
        qb.andWhere('donor.constituency_id IN (:...constituencyIds)', {
          constituencyIds: filters.constituencyId,
        });
      } else {
        qb.andWhere('donor.constituency_id = :constituencyId', {
          constituencyId: filters.constituencyId,
        });
      }
    }
    if (filters.subConstituencyId) {
      if (Array.isArray(filters.subConstituencyId)) {
        qb.andWhere('donor.sub_constituency_id IN (:...subConstituencyIds)', {
          subConstituencyIds: filters.subConstituencyId,
        });
      } else {
        qb.andWhere('donor.sub_constituency_id = :subConstituencyId', {
          subConstituencyId: filters.subConstituencyId,
        });
      }
    }
    if (filters.startDate) {
      qb.andWhere('donor.date_joined >= :startDate', {
        startDate: filters.startDate,
      });
    }
    if (filters.endDate) {
      qb.andWhere('donor.date_joined <= :endDate', {
        endDate: filters.endDate,
      });
    }
  }
