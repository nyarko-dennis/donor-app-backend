...
export class DonationCausesService {
  ...
  create(
    createDonationCauseDto: CreateDonationCauseDto,
  ): Promise<DonationCause> {
    const cause = this.donationCausesRepository.create(createDonationCauseDto);
    return this.donationCausesRepository.save(cause);
  }

  async findAll(
    pageOptionsDto: DonationCausesPageOptionsDto,
  ): Promise<PageDto<DonationCause>> {
    const queryBuilder =
      this.donationCausesRepository.createQueryBuilder('donation_cause');

    if (pageOptionsDto.search) {
      queryBuilder.where(
        'donation_cause.name ILIKE :search OR donation_cause.description ILIKE :search',
        {
    ...
    if (pageOptionsDto.isActive !== undefined) {
      queryBuilder.andWhere('donation_cause.is_active = :isActive', {
    ...
    queryBuilder
      .orderBy(
        `donation_cause.${pageOptionsDto.sortBy || 'created_at'}`,
  ...
  async update(
    id: string,
    updateDonationCauseDto: Partial<CreateDonationCauseDto>,
  ): Promise<DonationCause> {
