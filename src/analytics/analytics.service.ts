...
export class AnalyticsService {
  ...
  private applyDonationFilters(
    ...
    if (filter.campaignId) {
      if (Array.isArray(filter.campaignId)) {
        qb.andWhere('donation.campaign_id IN (:...campaignIds)', {
          campaignIds: filter.campaignId,
        });
      } else {
        qb.andWhere('donation.campaign_id = :campaignId', {
    ...
    if (filter.constituencyId) {
      if (Array.isArray(filter.constituencyId)) {
        qb.andWhere('donation.constituency_id IN (:...constituencyIds)', {
          constituencyIds: filter.constituencyId,
        });
      } else {
        qb.andWhere('donation.constituency_id = :constituencyId', {
  ...
  async getOverview(filter: AnalyticsFilterDto) {
    const donationQb = this.donationRepository
    ...
    const donorQb = this.donorRepository
      .createQueryBuilder('donor')
      .select('COUNT(donor.id)', 'totalDonors');

    if (filter.startDate) {
      donorQb.andWhere('donor.date_joined >= :startDate', {
        startDate: filter.startDate,
      });
    }
    if (filter.endDate) {
      donorQb.andWhere('donor.date_joined <= :endDate', {
        endDate: filter.endDate,
      });
    }
    if (filter.constituencyId) {
      if (Array.isArray(filter.constituencyId)) {
        donorQb.andWhere('donor.constituency_id IN (:...constituencyIds)', {
          constituencyIds: filter.constituencyId,
        });
      } else {
        donorQb.andWhere('donor.constituency_id = :constituencyId', {
          constituencyId: filter.constituencyId,
        });
      }
    }
