...
export class DonationsService implements OnModuleInit {
  ...
  async create(createDonationDto: CreateDonationDto): Promise<Donation> {
    ...
    const campaign = await this.campaignsRepository.findOneBy({
      id: campaignId,
    });
    if (!campaign) {
    ...
    if (donationData.donation_cause) {
      cause = await this.donationCausesRepository.findOneBy({
        name: donationData.donation_cause,
      });
      if (!cause) {
        ...
        throw new NotFoundException(
          `Donation Cause '${donationData.donation_cause}' not found`,
        );
      }
    }
  ...
  async findAll(
    pageOptionsDto: DonationsPageOptionsDto,
  ): Promise<PageDto<Donation>> {
    const queryBuilder =
      this.donationsRepository.createQueryBuilder('donation');
    ...
    if (pageOptionsDto.search) {
      queryBuilder.where(
        '(cause.name ILIKE :search OR donor.first_name ILIKE :search OR donor.last_name ILIKE :search)',
        {
    ...
    if (pageOptionsDto.donorId) {
      if (Array.isArray(pageOptionsDto.donorId)) {
        queryBuilder.andWhere('donation.donor.id IN (:...donorIds)', {
          donorIds: pageOptionsDto.donorId,
        });
      } else {
        queryBuilder.andWhere('donation.donor.id = :donorId', {
          donorId: pageOptionsDto.donorId,
        });
      }
    }
    ...
    if (pageOptionsDto.campaignId) {
      if (Array.isArray(pageOptionsDto.campaignId)) {
        queryBuilder.andWhere('donation.campaign.id IN (:...campaignIds)', {
          campaignIds: pageOptionsDto.campaignId,
        });
      } else {
        queryBuilder.andWhere('donation.campaign.id = :campaignId', {
          campaignId: pageOptionsDto.campaignId,
        });
      }
    }
    ...
    if (pageOptionsDto.causeId) {
      if (Array.isArray(pageOptionsDto.causeId)) {
        queryBuilder.andWhere('donation.cause.id IN (:...causeIds)', {
          causeIds: pageOptionsDto.causeId,
        });
      } else {
        queryBuilder.andWhere('donation.cause.id = :causeId', {
          causeId: pageOptionsDto.causeId,
        });
      }
    }
    ...
    if (pageOptionsDto.startDate) {
      queryBuilder.andWhere('donation.donation_date >= :startDate', {
        startDate: pageOptionsDto.startDate,
      });
    }

    if (pageOptionsDto.endDate) {
      queryBuilder.andWhere('donation.donation_date <= :endDate', {
    ...
    if (pageOptionsDto.payment_method) {
      if (Array.isArray(pageOptionsDto.payment_method)) {
        queryBuilder.andWhere(
          'donation.payment_method IN (:...paymentMethods)',
          {
    ...
    queryBuilder
      .orderBy(
        `donation.${pageOptionsDto.sortBy || 'donation_date'}`,
  ...
  async update(
    ...
    if (donation_cause) {
      const cause = await this.donationCausesRepository.findOneBy({
        name: donation_cause,
      });
      if (!cause) {
        throw new NotFoundException(
          `Donation Cause '${donation_cause}' not found`,
        );
      }
    ...
    if (constituency_id) {
      const constituency = await this.constituenciesRepository.findOneBy({
        id: constituency_id,
      });
      if (!constituency) {
        throw new NotFoundException(
          `Constituency with ID ${constituency_id} not found`,
        );
      }
    ...
    if (sub_constituency_id) {
      const subConstituency = await this.subConstituenciesRepository.findOneBy({
        id: sub_constituency_id,
      });
      if (!subConstituency) {
        throw new NotFoundException(
          `Sub-Constituency with ID ${sub_constituency_id} not found`,
        );
      }
  ...
  async initiateDonation(dto: InitiateDonationDto) {
    ...
    let donor = await this.donorsRepository.findOneBy({
      email: dto.donor.email,
    });

    let constituency: Constituency | null = null;
    if (dto.donor.constituency_id) {
      constituency = await this.constituenciesRepository.findOneBy({
        id: dto.donor.constituency_id,
      });
    }

    let subConstituency: SubConstituency | null = null;
    if (dto.donor.sub_constituency_id) {
      subConstituency = await this.subConstituenciesRepository.findOneBy({
        id: dto.donor.sub_constituency_id,
      });
    }
    ...
    if (!donor) {
      ...
      this.logger.log(
        `Created new donor: ${(donor as Donor).id} (${(donor as Donor).email})`,
      );
    }
    ...
    this.logger.log(
      `Donation ${savedDonation.id} created with reference: ${reference}`,
    );
